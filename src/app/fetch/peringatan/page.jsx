  ...
  return (
    <main style={mainStyle}>
      ...
      <header style={headerStyle}>
        <div style={headerContent}>
          <div style={logoSection}>
            ...
            <h1 style={titleStyle}>Peringatan Dini Bencana</h1>
          </div>
      ...
      <section style={detailSectionStyle}>
        ...
        {records.length === 0 && !error && <div style={loadingCard}>Memuat data...</div>}
        
        {records.map((record, i) => {
          const parsedBlocks = parsePeringatan(record.fields?.value || "");
          if (!Array.isArray(parsedBlocks)) return null;
          return parsedBlocks.map((parsed, idx) => (
            <div key={`${i}-${idx}`} style={detailCard}>
              <h2 style={detailCardTitle}>{parsed.lokasi}</h2>
              <p style={detailCardMeta}>
                <strong>Tanggal:</strong> {parsed.tanggal}
              </p>
              {parsed.entries && parsed.entries.length > 0 ? (
                parsed.entries.map((entry, j) => (
                  <div key={j} style={detailEntry}>
                    <p><strong>Pukul:</strong> {entry.waktu}</p>
                    <p><strong>Potensi:</strong> {entry.potensi}</p>
                    ...
                    {entry.aksi && entry.aksi.length > 0 && (
                      <>
                        <p><strong>Mitigasi & Aksi:</strong></p>
                        <ul style={detailList}>
                          {entry.aksi.map((a, ai) => (
                            <li key={ai} style={detailItem}>{a}</li>
                          ))}
                ...
                <p style={noDataText}>
                  Tidak ada data entries tersedia.
                </p>
              )}
            </div>
          ));
      ...
      {/* JSON Structure */}
      <section style={jsonSection}>
        <h2 style={jsonTitle}>Struktur Field (JSON)</h2>
        <div style={jsonCard}>
          <pre style={jsonPre}>{JSON.stringify(fieldObj, null, 2)}</pre>
        </div>
      </section>
    </main>
  );
